...
function renderKpis(cat,m){
 ...
 const period=activeMonth==="all"?"فروردین تا مرداد":months[+activeMonth];
 el("kpis").innerHTML=`
...
function renderCompare(cat,m){
 ...
 el("compare-panel").innerHTML=panelHead("۰۳","ترکیب فروش برندهای شرکت و سایر برندها","سایر برندهای بازار به‌صورت یک گروه تجمیع شده‌اند","کیلوگرم")+`<div class="compare-bars">${parts.map(p=>`<div class="compare-row"><span>${p.name}</span><div class="compare-track"><i style="--color:${colors[p.name]||'#7ba38f'};width:${p.value/max*100}%"></i></div><b>${fa(p.value)}</b><small>${fa(pct(p.value,market),1)}٪</small></div>`).join("")}</div>`;
}
...
function renderTable(cat,m){
 ...
 el("table-panel").innerHTML=panelHead("۰۴","جدول کنترل ماهانه","برندهای شرکت جدا از مجموع سایر برندها","کیلوگرم")+`<div class="table-scroll"><table><thead><tr><th>نوع</th><th>برند / گروه</th>${visibleMonths.map(x=>`<th>${x.name}</th>`).join("")}<th>جمع پنج‌ماهه</th><th>سهم انتخاب</th></tr></thead><tbody>${rows.map(r=>{const pv=periodValue(r.values);return `<tr><td><span class="brand-pill ${r.owned?'owned':'other'}">${r.owned?'شرکت':'سایر'}</span></td><td><b>${r.name}</b></td>${visibleMonths.map(x=>`<td>${fa(r.values[x.i])}</td>`).join("")}<td>${fa(sum(r.values))}</td><td><span class="share-pill">${fa(pct(pv,market),1)}٪</span></td></tr>`}).join("")}<tr class="total-row"><td colspan="2">کل گروه ${cat.name}</td>${visibleMonths.map(x=>`<td>${fa(m.totalMonthly[x.i])}</td>`).join("")}<td>${fa(sum(m.totalMonthly))}</td><td>۱۰۰٪</td></tr></tbody></table></div>`;
}
function render(){
 ...
 el("filter-description").textContent=`${activeBrand==="all"?'همه برندهای شرکت':activeBrand} · ${activeMonth==="all"?'پنج‌ماهه':months[+activeMonth]}`;
}
